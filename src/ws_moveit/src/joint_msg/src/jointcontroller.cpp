...
protected:
  ...
  joint_msg::joint_msg joint_motor_msg;
...
public:
    ...
    action_name_(name)
  {
    ...
    fore_arm_joint:4
    tool_joint:5 -->*/
    nh_.param("joint1", MotoName_Id["joint1"], 1);
    nh_.param("joint2", MotoName_Id["joint2"], 2);
    nh_.param("joint3", MotoName_Id["joint3"], 3);
    nh_.param("joint4", MotoName_Id["joint4"], 4);
    nh_.param("joint5", MotoName_Id["joint5"], 5);
  
 
    joint_pub = nh_.advertise<sensor_msgs::JointState>("/move_group/fake_controller_joint_states", 1);
 
    joint_motor_pub = nh_.advertise<joint_msg::joint_msg>("/arm_motors", 1000);
...
int main(int argc, char** argv)
{
  ...
  ROS_INFO("-------------xarm joint controller is running .");
  ros::spin();
